...
class Norway extends React.Component {
  ...
  componentDidMount() {
    const descrRef = db.ref("/casimir-cruises/destinations/norway/about");
    const activityRef = db.ref("casimir-cruises/destinations/norway/activities");
    const imageRef = storage.ref("/destinationPage/norway");
    let descr = [];
    let activities = [];
    let images = [];
    ...
    imageRef.child('/norway1.jpg').getDownloadURL().then((url) => {
      this.setState({
        image1: url
      })
    })
    imageRef.child('/norway2.jpg').getDownloadURL().then((url1) => {
      this.setState({
        image2: url1
      })
    })
    imageRef.child('/norway3.jpg').getDownloadURL().then((url2) => {
      this.setState({
        image3: url2
      })
    })
    imageRef.child('/norway4.jpg').getDownloadURL().then((url3) => {
      this.setState({
        image4: url3
      })
    })
