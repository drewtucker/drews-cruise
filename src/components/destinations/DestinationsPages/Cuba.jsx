...
class Cuba extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      description: "test",
  ...
  componentDidMount() {
    const descrRef = db.ref("/casimir-cruises/destinations/cuba/about");
    const activityRef = db.ref("casimir-cruises/destinations/cuba/activities");
    const imageRef = storage.ref("/destinationPage/cuba");
    let descr = [];
    let activities = [];
    let images = [];
    ...
    imageRef.child('/cuba1.jpg').getDownloadURL().then((url) => {
      this.setState({
        image1: url
      })
    })
    imageRef.child('/cuba2.jpg').getDownloadURL().then((url1) => {
      this.setState({
        image2: url1
      })
    })
    imageRef.child('/cuba3.jpg').getDownloadURL().then((url2) => {
      this.setState({
        image3: url2
      })
    })
    imageRef.child('/cuba4.jpg').getDownloadURL().then((url3) => {
      this.setState({
        image4: url3
      })
    })
