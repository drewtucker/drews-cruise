...
class NewZealand extends React.Component {
  ...
  componentDidMount() {
    const descrRef = db.ref("/casimir-cruises/destinations/new_zealand/about");
    const activityRef = db.ref("casimir-cruises/destinations/new_zealand/activities");
    const imageRef = storage.ref("/destinationPage/new_zealand");
    let descr = [];
    let activities = [];
    let images = [];
    ...
    imageRef.child('/new_zealand1.jpg').getDownloadURL().then((url) => {
      this.setState({
        image1: url
      })
    })
    imageRef.child('/new_zealand2.jpg').getDownloadURL().then((url1) => {
      this.setState({
        image2: url1
      })
    })
    imageRef.child('/new_zealand3.jpg').getDownloadURL().then((url2) => {
      this.setState({
        image3: url2
      })
    })
    imageRef.child('/new_zealand4.jpg').getDownloadURL().then((url3) => {
      this.setState({
        image4: url3
      })
    })
